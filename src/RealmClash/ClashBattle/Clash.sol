...
contract RealmClashBattleSystem {
    ...
    uint256 public constant VICTORY_XP = 50;
    uint256 public constant FORFEIT_XP = 10;
    ...
    function _tryMatchmaking() internal {
        if (waitingPlayers.length >= 2) {
            ...
            waitingPlayers[0] = waitingPlayers[waitingPlayers.length - 1];
            waitingPlayers.pop();
            waitingPlayers[0] = waitingPlayers[waitingPlayers.length - 1];
            waitingPlayers.pop();
    ...
    function _endBattle(uint256 _battleId, bool _isPlayer1Winner) internal {
        ...
        characterContract.updateBattleResult(
            losingCharacterId,
            false,
            wasForfeit ? FORFEIT_XP / 4 : VICTORY_XP / 4
        );
    ...
    function _calculateBattleDamage(
        ...
        uint256 baseDamage = _attackStat > _defenseStat
            ? _attackStat - (_defenseStat / 2)
            : _attackStat / 2;

        uint256 critChance = 5 + (_intelligence / 10);
        if (critChance > 30) critChance = 30;

        isCritical = _pseudoRandomNumber(_intelligence) % 100 < critChance;

        uint256 modifiedDamage = (baseDamage / 2) +
            (baseDamage * _damageMultiplier) /
            100;
